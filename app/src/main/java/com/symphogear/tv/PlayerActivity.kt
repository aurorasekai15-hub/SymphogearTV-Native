...
import com.symphogear.tv.databinding.ActivityPlayerBinding
import com.symphogear.tv.databinding.CustomControlBinding
import com.symphogear.tv.dialog.TrackSelectionDialog
...
class PlayerActivity : AppCompatActivity() {
    ...
    private fun playChannel() {
        ...
        // define mediaitem
        // Debug: log semua drmLicenses yang tersedia
        Log.d("DRM_DEBUG", "drmLicenses count: ${Playlist.cached.drmLicenses.size}")
        Playlist.cached.drmLicenses.forEach { Log.d("DRM_DEBUG", "  license name: ${it.name}") }
        ...
        Log.d("DRM_DEBUG", "matched drmLicense: $drmLicense")

        // FIXED: deteksi MimeType berdasarkan URL supaya ExoPlayer tahu format DASH vs HLS
        // Tanpa MimeType, ExoPlayer bisa gagal load DASH stream (.mpd) atau stream tanpa ekstensi jelas
        val mimeType = when {
            streamUrl.contains(".mpd", ignoreCase = true) -> MimeTypes.APPLICATION_MPD
            streamUrl.contains("manifest.mpd", ignoreCase = true) -> MimeTypes.APPLICATION_MPD
        ...
        // Base MediaItem builder dengan MimeType
        val baseBuilder = MediaItem.Builder()
            .setUri(Uri.parse(streamUrl))
            .also { if (mimeType != null) it.setMimeType(mimeType) }

        mediaItem = baseBuilder.build()

        if (!current?.drmName.isNullOrBlank() && drmLicense?.isNotEmpty() == true) {
            val isClearKey = current?.drmName?.startsWith("clearkey_") == true
            if (isClearKey) {
                // ClearKey: drmLicense = "keyid:key" atau "kid1:key1,kid2:key2"
                try {
                    val pairs = drmLicense.split(",")
                    val keysJson = StringBuilder()
                    keysJson.append("{\"keys\":[")
                    pairs.forEachIndexed { i, pair ->
                        val kv = pair.trim().split(":")
                        if (kv.size == 2) {
                            val kidHex = kv[0].trim()
                            val keyHex = kv[1].trim()
                            val kidB64 = android.util.Base64.encodeToString(
                                hexToBytes(kidHex), android.util.Base64.NO_PADDING or android.util.Base64.URL_SAFE or android.util.Base64.NO_WRAP)
                            val keyB64 = android.util.Base64.encodeToString(
                                hexToBytes(keyHex), android.util.Base64.NO_PADDING or android.util.Base64.URL_SAFE or android.util.Base64.NO_WRAP)
                            if (i > 0) keysJson.append(",")
                            keysJson.append("{\"kty\":\"oct\",\"kid\":\"$kidB64\",\"k\":\"$keyB64\"}")
                        }
                    }
                    keysJson.append("],\"type\":\"temporary\"}")
                    val licB64 = android.util.Base64.encodeToString(
                        keysJson.toString().toByteArray(), android.util.Base64.NO_WRAP)
                    val licUri = "data:application/json;base64,$licB64"
                    mediaItem = MediaItem.Builder()
                        .setUri(Uri.parse(streamUrl))
                        .also { if (mimeType != null) it.setMimeType(mimeType) }
                        .setDrmUuid(C.CLEARKEY_UUID)
                        .setDrmLicenseUri(licUri)
                        .setDrmMultiSession(false)
                        .build()
                    Log.d("DRM_DEBUG", "ClearKey mediaItem built OK, mimeType=$mimeType")
                } catch (e: Exception) {
                    Log.e("PlayerActivity", "ClearKey parse error: ${e.message}")
                    mediaItem = MediaItem.Builder()
                        .setUri(Uri.parse(streamUrl))
                        .also { if (mimeType != null) it.setMimeType(mimeType) }
                        .build()
                ...
                mediaItem = MediaItem.Builder()
                    ...
                    .build()
                Log.d("DRM_DEBUG", "Widevine mediaItem built, licUri=$drmLicense, mimeType=$mimeType")
            }
        } else if (!current?.drmName.isNullOrBlank() && drmLicense == null) {
            Log.e("DRM_DEBUG", "DRM channel '${current?.name}' but license NOT FOUND in drmLicenses!")
