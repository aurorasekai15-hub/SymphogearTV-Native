...
open class MainActivity : AppCompatActivity() {
    ...
    // Category name mapping (same as web version)
    ...
    private fun updateStats(playlistSet: Playlist) {
        try {
            val allChannels = playlistSet.categories
                ...
                ?: emptyList()
            binding.statTotal?.text = allChannels.size.toString()

            // Update current category title with first category
            val firstCat = playlistSet.categories?.firstOrNull()
            if (firstCat != null) {
                val key = firstCat.name?.lowercase()?.trim() ?: ""
                binding.textCurrentCat?.text = catNames[key] ?: "📺 ${firstCat.name?.uppercase()}"
            }
        } catch (e: Exception) { }
    }
